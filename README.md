...
Production editor data lives in Vercel Blob rather than the code repository, so normal GitHub pushes and Vercel deployments do not replace owner edits. Every successful save also writes an append-only, randomly suffixed history snapshot before updating the canonical timeline record. Canonical reads include the Blob upload timestamp as a cache-busting version, and the save API verifies the exact newly written timeline version through a unique URL before reporting success; this prevents an overwritten public Blob cache from hiding a saved line on the public view. Future schema changes must pass the save-preservation regression before deployment.
