...
The September 2026 typography-only refinement aligns the section heading with Research and Delivery at 38px / 500 on desktop, with a 34px phone cap and a 29px narrow-screen floor to preserve wrapping. Heading tracking is −0.05em, matching the shared project headline role. Final figures retain their large scale and 450 weight, adopt the shared metric tracking of −0.055em, and use normal numeral spacing. Labels use 13px regular text; closing emphasis uses 450 weight and −0.035em tracking. The existing Geist families and palette remain authoritative. These changes do not alter motion code, durations, easing, masks, hover rules, scroll geometry declarations, assets, or content.
