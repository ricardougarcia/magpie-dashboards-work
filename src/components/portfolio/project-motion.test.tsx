...
it("keeps section shells and focus stable while the mini-map follows scrolling, and disables parallax under reduced motion", async () => {
  ...
  const railLink = screen.getByRole("link", { name: "First" });
  const mapLink = screen.getByRole("link", { name: "01 First" });
  expect(screen.getByRole("navigation", { name: "Page mini-map" }).contains(mapLink)).toBe(true);
  expect(mapLink.getAttribute("href")).toBe("#first");
  expect(mapLink.getAttribute("aria-current")).toBe("location");
  railLink.focus();
  ...
  fireEvent.click(mapLink, { ctrlKey: true });
  expect(parseFloat(content.style.getPropertyValue("--section-shift"))).toBeGreaterThan(0);
  ...
  expect(content.style.getPropertyValue("--section-shift")).toBe("0px");
  expect(content.style.getPropertyValue("--section-opacity")).toBe("1");
  scroll = 2200; fireEvent.scroll(window); act(() => frames.splice(0).forEach((callback) => callback(0)));
  expect(mapLink.hasAttribute("aria-current")).toBe(false);
  expect(screen.getByRole("link", { name: "02 Second" }).getAttribute("aria-current")).toBe("location");
});
