...
export function MarketplaceBoardCluster({ entry }: { entry: BoardEntry }) {
  return <MarketplaceBoardMotion id={`region-${entry.id}`} data-region-code="MARKETPLACE" data-board-number={entry.number} data-board-state="ready" className={`${clusters.cluster} ${styles.cluster}`} aria-labelledby="marketplace-title">
    ...
    <ClusterEntrance className={styles.artifacts}>
      ...
      <div className={styles.orchestration}>
        <svg viewBox="0 0 120 68" aria-hidden="true" className={styles.convergence}>
          <path className={styles.baseTrace} pathLength="1" d="M 12 4 V 20 H 60 M 60 4 V 60 M 108 4 V 20 H 60" />
          <circle cx="12" cy="4" r="2" /><circle cx="60" cy="4" r="2" /><circle cx="108" cy="4" r="2" /><circle cx="60" cy="60" r="3" />
          <g className={styles.travelers}>
            <path pathLength="100" d="M 12 4 V 20 H 60 V 60" />
